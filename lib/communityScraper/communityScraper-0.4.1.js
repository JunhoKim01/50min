...
class CommunityScraper {
  ...
  //
  testParser() {
    const snapshotUrl = 'http://hgc.bestiz.net/zboard/zboard.php?id=ghm2b&page=1';
    const basePostUrl = 'http://hgc.bestiz.net/zboard/zboard.php?id=ghm2b&page=1&no=postId';
    ...
    for (let page = 1; page <= parserOptions.lastPage; page++) {
      ...
      postElement.each((index, post) => {
        ...
        matched.forEach((type) => {
          ...
          const item = {
            createdAt: Date.now(),
            communityName: 'ddanzi',    // Community name
            batchCount: this.batchCounter[communityName],
            itemId,
            type,                      // Matched type
            title,
