...
import TodayhumorParser from '../communityScraper/todayhumorParser.js';
...
export default class CommunityScraper {
  ...
  //
  testParser() {
    const snapshotUrl = 'http://www.todayhumor.co.kr/board/list.php?table=bestofbest&page=1';
    const basePostUrl = 'http://www.todayhumor.co.kr/board/view.php?table=bestofbest&no='; 
    // let content = null;
    // try {
    //   const result = 
    ...
    const postElement = $('td.subject').find('a');
  ...
  _parserBuilder(communityName, options) {
    const parsers = {
      ...
      todayhumor: () => new TodayhumorParser(options),
    };
