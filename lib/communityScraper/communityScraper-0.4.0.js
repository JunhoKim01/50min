...
CS = class CommunityScraperNew {
  ...
  // Public methods
  scrapStartWithOptions(options, callback) {
    if (! options) {
    ...
    // Log : Scarp start
    console.log(`[${options.type}] [${this.communityName}] Scrap started at : ${Date(batchStartTime)}`);
    ...
    this.batchHandle = Meteor.setInterval(() => {
      ...
      // Get just scrapped result
      result = this._getScrapByType(options.type, options.regexp, options.lastPage, batchCounter);
      ...
      for (let i = 0; i < result.length; i++) {
        if (postIdsInsertedBefore.indexOf(result[i].postId) === -1) {
          // Insert the itme only if the item is not saved already
          resultWithoutDup.push(result[i]);
          postIdsInsertedBefore.push(result[i].postId); // Save this post ID
        }
      ...
      //     // Insert the itme only if the item is not inserted
      //     resultWithoutDup.push(item);
      //     postIdsInsertedBefore.push(item.postId); // Save this post ID
      //   }
      // });
      // // Pass the results without Dup by callback function
      // callback(resultWithoutDup);

      

      // Log : Batch done
      console.log(`[${options.type}] [${this.communityName}] Batch # ${batchCounter}`);
      batchCounter += 1;
    }, batchIntervalMS);
  }
